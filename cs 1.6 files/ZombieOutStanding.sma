...
#define IP_LICENTIAT "89.40.104.27"
#define CHAT_TAG "[ZOMBIE.THEXFORCE.RO]"
#define STRING_NAME "The Zombie Apocalypse"
#define ZV_UNLIMITED (1<<2)

#define TASK_GAG 123

native zv_get_user_flags(id);
native zp_get_user_frost_balls(id);

new const FvaultName[] = "FvaultZombie";
new const FvaultName_Packs[] = "FvaultZombie_Packs";
new const RestrictedCvars[] = {
	"hostname",
	"rcon_password",
	"sv_downloadurl",
	"sv_allowdownload", 
...
new iPoints [ 33 ];
new g_UserGagTime[33];

new g_pCvarPoints;
new g_pCvarSave;
new g_pCvarReset;

new bool:g_bHaveGodMode[33];
new bool:g_frost[33];
new bool:g_UserHaveGag[33];
////////////////////////////////////////////////////////////////
new const ZP_CUSTOMIZATION_FILE[] = "SetariMod.ini"
new const ZP_EXTRAITEMS_FILE[] = "ExtraIteme.ini"
new const ZP_ZOMBIECLASSES_FILE[] = "ClaseZombie.ini"
...
// Plugin Version
new const PLUGIN_VERSION[] = "8 Feb Release"
...
cvar_randweapons, cvar_antidotelimit, cvar_madnesslimit,
...
cvar_lnjratio, cvar_lnjrespsurv, cvar_lnjrespnem  , cvar_flashsize2
...
public plugin_precache()
{
	...
	register_plugin("Zombie Plague", PLUGIN_VERSION, "MeRcyLeZZ, @bdul! ,93()|29!/<")
...
public plugin_init()
{
	...
	if(!equal(ip, IP_LICENTIAT))
		set_fail_state("[ZP] Nu detii o licenta valida a acestui addons. Contact Rainq");

	if(!g_pluginenabled || !g_zclass_i)
		set_fail_state("[ZOMBIE.THEXFORCE.RO] We found an error and we can't continue");


	register_event("DeathMsg", "Event_PlayerKilled", "a");

	register_clcmd("say /rs", "fwChatCmd_ResetScore");
	register_clcmd("say", "SayCheck");
	register_clcmd("say_team", "SayCheck");


	register_concmd("amx_give_packs", "fwCmd_GivePacks", ADMIN_ALL, "<user> <packs> - give user packs");
	register_concmd("amx_give_points", "fwCmd_GivePoints", ADMIN_ALL, "<user> <points> - give user points");
	register_concmd("amx_freeze", "fwCmd_FreezePlayer", ADMIN_KICK, "<user> - frozen a player");
	register_concmd("amx_unfreeze", "fwCmd_UnFreeze", ADMIN_KICK, "<user> - unfrozen a player");
	register_concmd("amx_gag", "fwCmd_GagPlayer", ADMIN_KICK, "<user> <reason> <minutes> - gag player");
	register_concmd("amx_ungag", "fwCmd_UngagPlayer", ADMIN_KICK, "<user> - remove player gag");
	register_concmd("amx_kick", "fwCmd_KickPlayer", ADMIN_KICK, "<user> <reason> - kick a player");
	register_concmd("amx_slay", "cmd_slay", ADMIN_SLAY, "<user> <reason> - slay a player");
	register_concmd("amx_slap", "cmd_slap", ADMIN_SLAY, "<user> <power> - slap a player");
	register_concmd("amx_cvar", "cmd_cvar", ADMIN_CVAR, "<cvar> <value> - set value cvar");
	register_concmd("amx_map", "cmd_map", ADMIN_MAP, "<mapname> - change current map");



	g_pCvarPoints = register_cvar("zp_points_for_kill", "2");
	g_pCvarSave = register_cvar("zp_packs_save", "1");
	g_pCvarReset = register_cvar("zp_reset_cost", "10");
	...
	// Language files
	register_dictionary("ZombieOutStanding_new.txt")
	...
	register_cvar("zp_mod_london", PLUGIN_VERSION, FCVAR_SERVER|FCVAR_SPONLY)
	set_cvar_string("zp_mod_london", PLUGIN_VERSION)
	...
	// Format mod name
	formatex(g_modname, charsmax(g_modname), "Zombie OutStanding");
...
public plugin_cfg() {
	if(g_pluginenabled) {
		...
		server_cmd("exec %s/ZombieOutStanding.cfg", strDir);
...
public logevent_round_end()
{
	...
	if (get_pcvar_num(cvar_statssave))
	{
		...
		for (id = 1; id <= g_maxplayers; id++)
		{
			...
			if(g_bHaveGodMode[id]) {
				set_user_godmode(id, 0);
				g_bHaveGodMode[id] = false;
			}

			if (is_user_connected(id) && g_frost[id])
			{
				g_frost[id]=false
...
public fw_PlayerSpawn_Post(id)
{
	...
	if(g_frost[id]) {
		new f = pev(id, pev_flags);
		if(f & FL_FROZEN) {
			f&=~FL_FROZEN;
			set_pev(id, pev_flags, f);
		}
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	if(g_frost[victim]) {
		new f = pev(victim, pev_flags);
		if(f & FL_FROZEN) {
			f&=~FL_FROZEN;
			set_pev(victim, pev_flags, f);
		}

		RemoveUserFreeze(victim);
	}
}
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	// Victim shouldn't take damage or victim is frozen
        if (g_frozen[victim] && !g_nemesis[attacker] || g_nodamage[victim]) 
                return HAM_SUPERCEDE;  
...
public fw_TakeDamage_Post(victim)
{
	...
	if (g_zombie[victim])
	{
		...
		if (!g_assassin[victim] && !g_nemesis[victim])
		{
	...
	else
	{
		...
		// Human
		if (!g_survivor[victim] && !g_sniper[victim]) return;
	}
...
public client_putinserver(id)
{
	...
	if (!is_user_bot(id))
	{
		...
		zp_colored_print(0, "^x01Player^x03 %s^x01 connected from [^x04%s^x01][^x04%s^x01]", g_playername[id], strCountry, strCity);

		new szData[16], szString[16];
		if(fvault_get_data(FvaultName, g_playername[id], szData, charsmax(szData)))
			iPoints[id] = str_to_num(szData);

		if(get_pcvar_num(g_pCvarSave)) {
			if(fvault_get_data(FvaultName_Packs, g_playername[id], szString, charsmax(szString)))
				g_ammopacks[id] = str_to_num(szString);
		}
	}
	else
	{
...
public fw_PlayerPreThink(id)
{
	...
	else
	{
		...
		else
		{
			...
			else if (!g_survivor[id] && !g_sniper[id])
...
///////////////////////////////
// Game Menu - LondoN Code  ///
///////////////////////////////
public show_menu_game(id) {
	new m[250], len = 0, f = get_user_flags(id);

	len += formatex(m[len], charsmax(m) - len, "\r%s^n", STRING_NAME);
	
	if(get_pcvar_num(cvar_buycustom))
		len += formatex(m[len], charsmax(m) - len, "\r[1] \wBuy Weapons^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\dBuy Weapons^n");

	if(get_pcvar_num(cvar_extraitems) & g_isalive[id])
		len += formatex(m[len], charsmax(m) - len, "\r[2] \wBuy Extra Items^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\dBuy Extra Items^n");

	len += formatex(m[len], charsmax(m) - len, "\r[3] \wBuy Features with Points^n");

	if(get_pcvar_num(cvar_zclasses))
		len += formatex(m[len], charsmax(m) - len, "\r[4] \wChoose Zombie Class^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\dChoose Zombie Class^n");

	if(g_isalive[id])
		len += formatex(m[len], charsmax(m) - len, "\r[5] \wUnstruck^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\dUnstruck^n");

	if(!g_isalive[id] || !get_pcvar_num(cvar_blocksuicide) || (f & g_access_flag[ACCESS_ADMIN_MENU]))
		len += formatex(m[len], charsmax(m) - len, "\r[6] \wJoin Spectators^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\dJoin Spectators^n");

	if(f & g_access_flag[ACCESS_ADMIN_MENU3])
		len += formatex(m[len], charsmax(m) - len, "\r[7] \wAdmin Menu^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[7] Admin Menu^n");

	len += formatex(m[len], charsmax(m) - len, "^n^n\r0 - EXIT");
	show_menu(id, KEYSMENU, m, -1, "Game Menu");
}
...
show_menu_extras(id)
{
	...
	formatex(menu, charsmax(menu), "\rBuy Extra Items^n");
	...
	for (item = 0; item < g_extraitem_i; item++)
	{
		...
		formatex(menu, charsmax(menu), "%s \r(%d ammo packs)", buffer, ArrayGetCell(g_extraitem_cost, item));
		buffer[0] = item
		buffer[1] = 0
...
public show_menu_zclass(id)
{
	...
	// Title
	formatex(menu, charsmax(menu), "\rChoose Zombie Class^n");
...
////////////////////////////////////////////////////////////
///	Admin Menu - LondoN eXtream code		///
///////////////////////////////////////////////////////////
public show_menu_admin(id) {
	new m[250], len = 0, f = get_user_flags(id);

	len += formatex(m[len], charsmax(m) - len, "\rChance Class or Respawn Someone^n^n");

	if(f & (g_access_flag[ACCESS_MODE_INFECTION] | g_access_flag[ACCESS_MAKE_ZOMBIE] | g_access_flag[ACCESS_MAKE_HUMAN]))
		len += formatex(m[len], charsmax(m) - len, "\r[1] \wMake Zombie or Human^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[1] Make Zombie or Human^n");

	if(f & (g_access_flag[ACCESS_MODE_NEMESIS] | g_access_flag[ACCESS_MAKE_NEMESIS]))
		len += formatex(m[len], charsmax(m) - len, "\r[2] \wMake Nemesis^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[2] Make Nemesis^n");

	if(f & (g_access_flag[ACCESS_MODE_SURVIVOR] | g_access_flag[ACCESS_MAKE_SURVIVOR]))
		len += formatex(m[len], charsmax(m) - len, "\r[3] \wMake Survivor^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[3] Make Survivor^n");

	if(f & (g_access_flag[ACCESS_MODE_SNIPER] | g_access_flag[ACCESS_MAKE_SNIPER]))
		len += formatex(m[len], charsmax(m) - len, "\r[4] \wMake Sniper^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[4] Make Sniper^n");

	if(f & (g_access_flag[ACCESS_MODE_ASSASSIN] | g_access_flag[ACCESS_MAKE_ASSASSIN]))
		len += formatex(m[len], charsmax(m) - len, "\r[5] \wMake Assassin^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[5] Make Assassin^n");

	if(f & g_access_flag[ACCESS_RESPAWN_PLAYERS])
		len += formatex(m[len], charsmax(m) - len, "\r[6] \wRespawn Someone^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[6] Respawn Someone^n");

	len += formatex(m[len], charsmax(m) - len, "^n\r9 - NEXT");
	len += formatex(m[len], charsmax(m) - len, "^n\r0 - BACK");
	
	show_menu(id, KEYSMENU, m, -1, "Admin Menu");
}

public show_menu2_admin(id) {
	new m[250], len = 0, f = get_user_flags(id);

	len += formatex(m[len], charsmax(m) - len, "\rStart New Mode^n^n");

	if((f & g_access_flag[ACCESS_MODE_MULTI]) && allowed_multi())
		len += formatex(m[len], charsmax(m) - len, "\r[1] \wMultiple Infection^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[1] Multiple Infection^n");

	if((f & g_access_flag[ACCESS_MODE_SWARM]) && allowed_swarm())
		len += formatex(m[len], charsmax(m) - len, "\r[2] \wSwarm Infection^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[2] Swarm Infection^n");

	if((f & g_access_flag[ACCESS_MODE_PLAGUE]) && allowed_plague())
		len += formatex(m[len], charsmax(m) - len, "\r[3] \wPlague Mode^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[3] Plague Mode^n");

	if((f & g_access_flag[ACCESS_MODE_LNJ]) && allowed_lnj())
		len += formatex(m[len], charsmax(m) - len, "\r[4] \wArmageddon Mode^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[4] Armageddon Mode^n");

	
	len += formatex(m[len], charsmax(m) - len, "^n\r9 - NEXT");
	len += formatex(m[len], charsmax(m) - len, "^n\r0 - BACK");

	show_menu(id, KEYSMENU, m, -1, "Menu2 Admin");
}

public show_menu3_admin(id) {
	new m[245], len = 0, f = get_user_flags(id);

	len += formatex(m[len], charsmax(m) - len, "\rAdministrator Menu^n^n");

	if(f & g_access_flag[ACCESS_ADMIN_MENU])
		len += formatex(m[len], charsmax(m) - len, "\r[1] \wChance Class / Respawn Someone^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[1] Chance Class / Respawn Someone^n");

	len += formatex(m[len], charsmax(m) - len, "\r[2] \wStart New Mode^n");

	if(f & g_access_flag[ACCESS_ENABLE_MOD])
		len += formatex(m[len], charsmax(m) - len, "\r[3] \wDisable Zombie Mode^n");
	else
		len += formatex(m[len], charsmax(m) - len, "\d[3] Disable Zombie Mode^n");

	len += formatex(m[len], charsmax(m) - len, "^n\r0 - EXIT");
	show_menu(id, KEYSMENU, m, -1, "Menu3 Admin");
}

public show_menu4_admin(id) {
	new m[240], len = 0;

	len += formatex(m[len], charsmax(m) - len, "\rDisable Zombie Mode?^n^n");
	len += formatex(m[len], charsmax(m) - len, "\r[1] \wYES^n");
	len += formatex(m[len], charsmax(m) - len, "\r[2] \wNO^n^n");
	len += formatex(m[len], charsmax(m) - len, "\r0 - EXIT");
	show_menu(id, KEYSMENU, m, -1, "Menu4 Admin");
}
			...
			case ACTION_ZOMBIEFY_HUMANIZE: // Zombiefy/Humanize command
			{
				if (g_zombie[player])
				{
					if (allowed_human(player) && (userflags & g_access_flag[ACCESS_MAKE_HUMAN]))
					{
						...
						if (!g_nemesis[player] && !g_assassin[player])
...
public menu_game(id, key) {
	switch(key) {
		case 0: {
			if(get_pcvar_num(cvar_buycustom)) {
				WPN_AUTO_ON = 0;
				zp_colored_print(id, "^x04[ZP]^x01 Your buy menu has been re-enabled!");
		...
		case 1: {
			if(get_pcvar_num(cvar_extraitems)) {
				if(g_isalive[id])
					...
					zp_colored_print(id, "^x04[ZP]^x01 You need to be alive to buy extra items.");
			}
		}

		case 2: show_menu_points(id);
		case 3: {
			if(get_pcvar_num(cvar_zclasses))
				show_menu_zclass(id);
			else
				zp_colored_print(id, "^x04[ZP]^x01 Zombie Classes are unavaible right now.");
		}

		case 4: {
			if(g_isalive[id] && is_player_stuck(id)) {
		...
		case 5: {
			if(g_isalive[id]) {
				if(get_pcvar_num(cvar_blocksuicide) && !(get_user_flags(id) & g_access_flag[ACCESS_ADMIN_MENU])) {
					zp_colored_print(id, "^x04[ZP]^x01 You can't transfer to the spectators!");
					return PLUGIN_HANDLED;
				}
...
////////////////////////////////////////////////////
//	London Code - Welcome Message		////
////////////////////////////////////////////////////
public welcome_msg() {
	new A, P;
	new ALeader = -1, PLeader = -1;
	new maxA = -10000, maxP = -10000;
	new g_Packs[16], g_Points[16];
	...
	zp_colored_print(0, "^x03%s^x04 ZOMBIE.THEXFORCE.RO va ureaza^x03 joc placut!", STRING_NAME);
	
	if((ALeader != -1 && maxA >=1) && (PLeader != -1 && maxP >= 1)) {
		AddCommas(maxA, g_Packs, charsmax(g_Packs));
		AddCommas(maxP, g_Points, charsmax(g_Points));

		zp_colored_print(0, "^x04[ZP]^x03 %s^x01 is leader with^x04 %s^x01 ammo packs.", g_playername[ALeader], g_Packs);
		zp_colored_print(0, "^x04[ZP]^x03 %s^x01 is leader with^x04 %s^x01 points.", g_playername[PLeader], g_Points);
	}

	set_hudmessage(0, 127, 255, HUD_EVENT_X, HUD_EVENT_Y, 1, 0.0, 3.0, 2.0, 1.0, -1)
	ShowSyncHudMsg(0, g_MsgSync, "BE CAREFULL!^nBiological arm has been lost!");
}
...
public respawn_player_task(taskid)
{
	...
	if (!g_endround && team != FM_CS_TEAM_SPECTATOR && team != FM_CS_TEAM_UNASSIGNED && !g_isalive[ID_SPAWN] && (!g_survround || get_pcvar_num(cvar_allowrespawnsurv)) && (!g_swarmround || get_pcvar_num(cvar_allowrespawnswarm)) && (!g_nemround || get_pcvar_num(cvar_allowrespawnnem))
	&& (!g_plagueround || get_pcvar_num(cvar_allowrespawnplague)) && (!g_sniperround || get_pcvar_num(cvar_allowrespawnsniper)) && (!g_assassinround || get_pcvar_num(cvar_allowrespawnassassin)) && (!g_lnjround || get_pcvar_num(cvar_allowrespawnlnj)))
	{
		// Infection rounds = none of the above
		if (!get_pcvar_num(cvar_allowrespawninfection) && !g_survround && !g_nemround && !g_swarmround && !g_plagueround && !g_sniperround && !g_assassinround && !g_lnjround)
			return;
...
check_round(leaving_player)
{
	...
	if (g_zombie[leaving_player] && fnGetZombies() == 1)
	{
		...
		if (g_assassin[leaving_player])
			zombieme(id, 0, 0, 0, 0, 1)
		if (!g_assassin[leaving_player] && !g_nemesis[leaving_player] )
			zombieme(id, 0, 0, 0, 0, 0)
	...
	else if (!g_zombie[leaving_player] && fnGetHumans() == 1)
	{
		...
		if (g_sniper[leaving_player])
			humanme(id, 0, 0, 1)
		if (!g_survivor[leaving_player] && !g_sniper[leaving_player])
			humanme(id, 0, 0, 0)
	...
	switch (weaponid)
	{
		case CSW_KNIFE: // Custom knife models
		{
			if (g_zombie[id])
			{
				...
				if (!g_nemesis[id] && !g_assassin[id] ) // Zombies
				{
					// Admin knife models?
...
public ShowHUD(taskid) {
	...
	if(Player != ID_SHOWHUD) {
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%s Spectating: %s^n%s HP: %s AP: %d Packs: %s Points: %s^n%s, %s", (zv_get_user_flags(Player) & ZV_UNLIMITED) ? "(Gold MemberÂ®)" : "", g_playername[Player], g_pClass, strHealth, get_user_armor(Player), strPacks, strPoints, strCountry, strCity);
	}
	else {
		...
		new strFrost[64];
		formatex(strFrost, charsmax(strFrost), "Frost Balls: %d (Press R to freeze human)", zp_get_user_frost_balls(ID_SHOWHUD));
...
command_zombie(id, player)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 tuned^x03 %s^x01 into a zombie.", g_playername[id], g_playername[player]);
...
command_human(id, player)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 turned^x03 %s^x01 into a human.", g_playername[id], g_playername[player]);
...
command_survivor(id, player)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 turned^x03 %s^x01 into a survivor.", g_playername[id], g_playername[player]);
...
command_nemesis(id, player)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 turned^x03 %s^x01 into a Nemesis.", g_playername[id], g_playername[player]);
...
command_respawn(id, player)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 respawned^x03 %s", g_playername[id], g_playername[player]);
...
command_swarm(id)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 started Swarm Mode!", g_playername[id]);
...
command_multi(id)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 started Multiple Infection!", g_playername[id]);
...
command_plague(id)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 started Plague Mode!", g_playername[id]);
...
command_sniper(id, player)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 turned^x03 %s^x01 into a Sniper.", g_playername[id], g_playername[player]);
...
command_assassin(id, player)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 turned^x03 %s^x01 into an Assassin", g_playername[id], g_playername[player]);
...
command_lnj(id)
{
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 started Armageddon Mode!", g_playername[id]);
...
public native_register_zombie_class(const name[], const info[], const model[], const clawmodel[], hp, speed, Float:gravity, Float:knockback)
{
	...
	for (i = 0; i < size; i++)
	{
		...
		// Precache clawmodel
		formatex(prec_mdl, charsmax(prec_mdl), "models/ZombieOutStanding/%s", buffer)
...
public showStatus(id)
{
	if(!is_user_bot(id) && is_user_connected(id)) 
	{
		...
		if (team1 == team2)    // friend
		{
			...
			ShowSyncHudMsg(id, g_status_sync,"%L", LANG_PLAYER, "AIM_INFO", name, get_user_health(pid), get_user_armor(pid), g_ammopacks[pid])
		} 
	}
}
...
public client_disconnect(id) {
	if(!is_user_bot(id) || !is_user_hltv(id)) {
		new a[16], b[16];
		num_to_str(iPoints[id], a, sizeof(a)-1);
		fvault_set_data(FvaultName, g_playername[id], a);
		if(get_pcvar_num(g_pCvarSave)) {
			num_to_str(g_ammopacks[id], b, sizeof(b)-1);
			fvault_set_data(FvaultName_Packs, g_playername[id], b);
		}
	}
	return PLUGIN_CONTINUE;
}
...
public Event_PlayerKilled() {
	...
	if(a == b || !is_user_alive(a))
		return PLUGIN_HANDLED;

	iPoints[a] += get_pcvar_num(g_pCvarPoints);
	if(iPoints[b] > 1)
		iPoints[b] -= 1;
	return PLUGIN_CONTINUE;
}

public fwCmd_GivePacks(Player, Level, Cid) {
	if(!cmd_access(Player, Level, Cid, 2))
		return PLUGIN_HANDLED;

	new strPlayer[32], strAmount[16], g_Commas[16];
	read_argv(1, strPlayer, charsmax(strPlayer));
	read_argv(2, strAmount, charsmax(strAmount));

	AddCommas(str_to_num(strAmount), g_Commas, charsmax(g_Commas));

	new iTarget = cmd_target(Player, strPlayer, 8);

	if(!iTarget)	return PLUGIN_HANDLED;

	g_ammopacks[iTarget] += str_to_num(strAmount);
	zp_colored_print(0, "^x04[ZP]^x01 Admin^x03 %s^x01 gived^x04 %s^x01 packs to^x03 %s", g_playername[Player], g_Commas, g_playername[iTarget]);
	return PLUGIN_CONTINUE;
}

public fwCmd_GivePoints(Player, Level, Cid) {
	if(!cmd_access(Player, Level, Cid, 2))
		return PLUGIN_HANDLED;
	...
	iPoints[iTarget] += str_to_num(strAmount);
	zp_colored_print(0, "^x04[ZP]^x01 Admin^x03 %s^x01 gived^x04 %s^x01 points to^x03 %s", g_playername[Player], g_Commas, g_playername[iTarget]);
	return PLUGIN_CONTINUE;
}

public show_menu_points(id) {
	new mx = menu_create("\rBuy Features with Points^n", "menu_points");
	new p = iPoints[id];

	// Buy 100 Ammo Packs
	if(p < 120)
		menu_additem(mx, "\dBuy 100 ammo packs [120 Points]");
	else
		menu_additem(mx, "\wBuy 100 ammo packs \r[120 Points]");

	// Buy 200 Ammo Packs
	if(p < 200)
		menu_additem(mx, "\dBuy 200 ammo packs [200 Points]");
	else
		menu_additem(mx, "\wBuy 200 ammo packs \r[200 Points]");

	// Buy 300 Ammo Packs
	if(p < 260)
		menu_additem(mx, "\dBuy 300 ammo packs [260 Points]");
	else
		menu_additem(mx, "\wBuy 300 ammo packs \r[260 Points]");

	// Buy God Mode
	if(p < 400)
		menu_additem(mx, "\dBuy God Mode [400 Points]");
	else
		menu_additem(mx, "\wBuy God Mode \r[400 Points]");

	// Buy Chat Tag
	if(p < 700)
		menu_additem(mx, "\dBuy Chat Tag [700 Points]");
	else
		menu_additem(mx, "\wBuy Chat Tag \r[700 Points]");

	// Buy Slot
	if(p < 750)
		menu_additem(mx, "\dBuy Slot [750 Points]");
	else
		menu_additem(mx, "\wBuy Slot \r[750 Points]");

	// Buy Gold Member
	if(p < 8000)
		menu_additem(mx, "\dBuy Gold Member Â® [8,000 Points]");
	else
		menu_additem(mx, "\wBuy Gold Member Â® \r[8,000 Points]");

	menu_setprop(mx, MPROP_EXIT, MEXIT_ALL);
	menu_display(id, mx, 0);
	return PLUGIN_HANDLED;
}

public menu_points(id, mx, item) { 
	if(item == MENU_EXIT) {
		menu_destroy(mx);
		return PLUGIN_HANDLED;
	}
	...
	switch(item)
	{
		case 0: {
			if(iPoints[id] < 120) {
				zp_colored_print(id, "^x04[ZP]^x01 You don't have enought points to buy^x03 100 ammo packs");
				return PLUGIN_HANDLED;
			}

			g_ammopacks[id] += 100;
			iPoints[id] -= 120;
			zp_colored_print(id, "^x04[ZP]^x01 You bought^x03 100 ammo packs^x01!");
		}


		case 1: {
			if(iPoints[id] < 200) {
				zp_colored_print(id, "^x04[ZP]^x01 You don't have enought points to buy^x03 200 ammo packs");	
				return PLUGIN_HANDLED;
			}

			g_ammopacks[id] += 200;
			iPoints[id] -= 200;
			zp_colored_print(id, "^x04[ZP]^x01 You bought^x03 200 ammo packs^x01!");
		}

		case 2: {
			if(iPoints[id] < 260) {
				zp_colored_print(id, "^x04[ZP]^x01 You don't have enought points to buy^x03 300 ammo packs");
				return PLUGIN_HANDLED;
			}
			...
			g_ammopacks[id] += 300;
			iPoints[id] -= 260;
			zp_colored_print(id, "^x04[ZP]^x01 You bought^x03 300 ammo packs^x01!");
		}

		case 3: {
			if(iPoints[id] < 400) {
				zp_colored_print(id, "^x04[ZP]^x01 You don't have enought points to buy^x03 God Mode");
				return PLUGIN_HANDLED;
			}

			if(g_bHaveGodMode[id]) {
				zp_colored_print(id, "^x04[ZP]^x01 You already own this!");
				return PLUGIN_HANDLED;
			}

			set_user_godmode(id, 1);
			iPoints[id] -= 400;
			g_bHaveGodMode[id] = true;
		}

		case 4: zp_colored_print(id, "^x04[ZP]^x01 Section under construction!");
		case 5: {
			if(iPoints[id] < 750) {
				zp_colored_print(id, "^x04[ZP]^x01 You don't have enought points to buy^x03 Server Slot");
				return PLUGIN_HANDLED;
			}

			if(get_user_flags(id) & ADMIN_RESERVATION) {
				zp_colored_print(id, "^x04[ZP]^x01 You already own slot on this server");
				return PLUGIN_HANDLED;
			}


			new g_Password[6], strLine[256];
			formatex(g_Password, charsmax(g_Password), "%d%d%d%d%d%d", random_num(1,9), random_num(1,9), random_num(1,9), random_num(1,9), random_num(1,9), random_num(1,9));
			formatex(strLine, charsmax(strLine), "^"%s^" ^"%s^" ^"b^" ^"a^"", g_playername[id], g_Password);
			write_file("addons/amxmodx/configs/users.ini", strLine);

			zp_colored_print(id, "^x04-------------------------------------------------------------------");
			zp_colored_print(id, "^x03[ZP]^x01 You bought^x04 Server SLOT!");
			zp_colored_print(id, "^x03[ZP]^x01 Write^x04 setinfo _pw %s^x01 in console for login", g_Password);
			zp_colored_print(id, "^x04--------------------------------------------------------------------");


			console_print(id, "SERVER SLOT PASSWORD: %s", g_Password);
			console_print(id, "SERVER SLOT PASSWORD: %s", g_Password);
			console_print(id, "SERVER SLOT PASSWORD: %s", g_Password);
		...
		case 6: {
			if(iPoints[id] < 8000) {
				zp_colored_print(id, "^x04[ZP]^x01 You don't have enought points to buy^x03 Gold Member");
				return PLUGIN_HANDLED;
			}

			if(get_user_flags(id) & ADMIN_RESERVATION) {
				zp_colored_print(id, "^x04[ZP]^x01 You already own Gold Member on this server");
				return PLUGIN_HANDLED;
			}
			...
			new g_Password[6], strLine[256];
			formatex(g_Password, charsmax(g_Password), "%d%d%d%d%d%d", random_num(1,9), random_num(1,9), random_num(1,9), random_num(1,9), random_num(1,9), random_num(1,9));
			formatex(strLine, charsmax(strLine), "^"%s^" ^"%s^" ^"abcdef^" ^"a^"", g_playername[id], g_Password);
			write_file("addons/amxmodx/configs/vips.ini", strLine);

			zp_colored_print(id, "^x04-------------------------------------------------------------------");
			zp_colored_print(id, "^x03[ZP]^x01 You bought^x04 Gold Member!");
			zp_colored_print(id, "^x03[ZP]^x01 Write^x04 setinfo _pw %s^x01 in console for login", g_Password);
			zp_colored_print(id, "^x04--------------------------------------------------------------------");


			console_print(id, "GOLD MEMBER PASSWORD: %s", g_Password);
			console_print(id, "GOLD MEMBER PASSWORD: %s", g_Password);
			console_print(id, "GOLD MEMBER PASSWORD: %s", g_Password);
...
public fwChatCmd_ResetScore(id) {
	if(g_ammopacks[id] < get_pcvar_num(g_pCvarReset)) {
		zp_colored_print(id, "^x04[ZP]^x01 You need^x03 %d^x01 ammo packs to reset your score!", get_pcvar_num(g_pCvarReset));
		return PLUGIN_HANDLED;
	}

	if(!get_user_frags(id) || !cs_get_user_deaths(id)) {
		zp_colored_print(id, "^x04[ZP]^x01 We can't reset your score because you have^x03 0^x01 kills and^x03 0^x01 deaths.");
		return PLUGIN_HANDLED;
	}
	...
	g_ammopacks[id] -= 10;
	zp_colored_print(id, "^x04[ZP]^x01 Your stats has been reset!");
	return PLUGIN_CONTINUE;
}

public fwCmd_FreezePlayer(id, level, cid) {
	if(!cmd_access(id, level, cid, 2))
		return PLUGIN_HANDLED;

	new strPlayer[32], Player;
	read_argv(1, strPlayer, charsmax(strPlayer));
	Player = cmd_target(id, strPlayer, CMDTARGET_ONLY_ALIVE|CMDTARGET_ALLOW_SELF);

	if(!Player)
		return PLUGIN_HANDLED;

	g_frost[Player] = true;
	new flags=pev(Player,pev_flags)
	if (!(flags&FL_FROZEN))
		flags|=FL_FROZEN,set_pev(Player,pev_flags,flags)

	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 has frozen^x03 %s", g_playername[id], g_playername[Player]);
	return PLUGIN_CONTINUE;
}

public fwCmd_UnFreeze(id, level, cid) {
	if(!cmd_access(id, level, cid, 2)) 
		return PLUGIN_HANDLED;

	new str[32], p;
	read_argv(1, str, charsmax(str));
	p = cmd_target(id, str, CMDTARGET_ONLY_ALIVE|CMDTARGET_ALLOW_SELF);

	if(!p)	return PLUGIN_HANDLED;

	g_frost[p] = false;
	new f = pev(p, pev_flags);

	if(f & FL_FROZEN) {
		f&=~FL_FROZEN;
		set_pev(p, pev_flags, f);
	}

	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 has unfrozen^x03 %s", g_playername[id], g_playername[p]);
	return PLUGIN_CONTINUE;
}

RemoveUserFreeze(id) {
	static Float:vec[3], Float:amt
	entity_get_vector(id,EV_VEC_rendercolor,vec)
	amt=entity_get_float(id,EV_FL_renderamt)

	if (vec[0]==0.0 && vec[1]==90.0 && vec[2]==128.0&&amt==30.0)
		fm_set_rendering(id)
}

public fwCmd_GagPlayer(id, level, cid) {
	if(!cmd_access(id, level, cid, 4)) 
		return PLUGIN_HANDLED;

	new strPlayer[32], strReason[32], strMinutes[6];
	new g_target;
	read_argv(1, strPlayer, charsmax(strPlayer));
	...
	g_target = cmd_target(id, strPlayer, 10);

	if(!g_target)	return PLUGIN_HANDLED;

	if(g_UserHaveGag[g_target]) {
		console_print(id, "[ZOMBIE.THEXFORCE.RO] This player is already gagged");
		return PLUGIN_HANDLED;
	}

	g_UserHaveGag[g_target] = true;
	g_UserGagTime[g_target] = str_to_num(strMinutes);

	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 have gagged^x03 %s^x01 for^x03 %d^x01 minutes due to^x03 %s", g_playername[id], g_playername[g_target], str_to_num(strMinutes), strReason);
	set_task(60.0, "gag_count", g_target+TASK_GAG, _, _, "b");
	return PLUGIN_CONTINUE;
}

public fwCmd_UngagPlayer(id, level, cid) {
	if(!cmd_access(id, level, cid, 2))
		return PLUGIN_HANDLED;

	new strPlayer[32], g_target;
	read_argv(1, strPlayer, charsmax(strPlayer));
	g_target = cmd_target(id, strPlayer, 11);

	if(!g_target)	return PLUGIN_HANDLED;

	if(!g_UserHaveGag[g_target]) {
		console_print(id, "[ZOMBIE.THEXFORCE.RO] Jucatorul nu are gag!");
		return PLUGIN_HANDLED;
	}

	remove_task(g_target+TASK_GAG);
	g_UserHaveGag[g_target] = false;
	g_UserGagTime[g_target] = 0;
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 has ungagged^x03 %s", g_playername[id], g_playername[g_target]);
	return PLUGIN_CONTINUE;
}

public gag_count(task) {
	new i = task - TASK_GAG;
	...
	if(g_UserGagTime[i] <= 0) {
		remove_task(i+TASK_GAG);
		g_UserHaveGag[i] = false;
		g_UserGagTime[i] = 0;
		zp_colored_print(i, "^x04[ZP]^x01 You have been ungagged. You can't chat again");
...
public SayCheck(id) {
	...
	if(g_UserHaveGag[id]) {
		zp_colored_print(id, "^x04[ZP]^x01 You can't use the chat until gag expires.");
		return PLUGIN_HANDLED;
	}
...
public fwCmd_KickPlayer(id, level, cid) {
	if(!cmd_access(id, level, cid, 3))
		return PLUGIN_HANDLED;

	new str[32], reason[32], target;
	read_argv(1, str, charsmax(str));
	read_argv(2, reason, charsmax(reason));

	remove_quotes(reason);
	target = cmd_target(id, str, 8);

	if(!target)	return PLUGIN_HANDLED;
	
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 has kicked^x03 %s^x01 due to^x03 %s", g_playername[id], g_playername[target], equal(reason, "") ? "N/A" : reason);
...
public cmd_slay(id, level, cid) {
	...
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 slayed^x03 %s^x01 due to^x03 %s", g_playername[id], g_playername[target], equal(reason, "") ? "N/A" : reason);
...
public cmd_slap(id, level, cid) {
	...
	zp_colored_print(0, "^x04[ZP]^x03 %s^x01 slapped^x03 %s^x01 with^x03 %d^x01 power.", g_playername[id], g_playername[target], str_to_num(power));
	user_slap(target, str_to_num(power));
	return PLUGIN_CONTINUE;
}
...
public cmd_cvar(id, level, cid) {
	...
	for(new i = 0; i < sizeof(RestrictedCvars); i++) {
		if(equal(cvar, RestrictedCvars[i])) {
			console_print(id, "[ZP] You can't modify this cvar value!");
			break;
		}
		else {
			set_cvar_string(cvar, value);
			zp_colored_print(0, "^x04[ZP]^x03 %s^x01 changed value of cvar^x03 %s", g_playername[id], cvar);
		}
	}
...
{ 
    ...
    { 
            log_error(AMX_ERR_NATIVE, "[Debug] Invalid user (%d)", id) 
            return false; 
    } 
