...
/*
 * 使用api需要将 ZombieOutstandingAPI.dll 放在插件同目录下 一起编译 /To use the API, place ZombieOutstandingAPI.dll  in the same directory as the plugin to Compile.
 * 需要在csproj内新增 / Need to add to csproj :
 * 
 * <ItemGroup> 
 * <Reference Include="ZombieOutstandingCS2"> 
 * <HintPath>ZombieOutstandingAPI.dll
 * </HintPath> 
 * </Reference> 
 * </ItemGroup>
 */
...
{
    private IZombieOutstandingAPI? _zpApi; //创建api实例, Create an API instance
    ...
    public override void UseSharedInterface(IInterfaceManager interfaceManager)
    {
        if (interfaceManager.HasSharedInterface("HanZombiePlague")) //获取api  Get API
        {
            ...
            _zpApi.ZO_OnGameStart += (bool gamestart) =>
            {
            ...
            _zpApi.ZO_OnPlayerInfect += (IPlayer attacker, IPlayer player, bool grenade, string zombieName) =>
            {
            ...
            _zpApi.ZO_OnMotherZombieSelected += (IPlayer player) =>
            {
            ...
            _zpApi.ZO_OnNemesisSelected += (IPlayer player) =>
            {
            ...
            _zpApi.ZO_OnAssassinSelected += (IPlayer player) =>
            {
            ...
            _zpApi.ZO_OnHeroSelected += (IPlayer player) =>
            {
            ...
            _zpApi.ZO_OnSurvivorSelected += (IPlayer player) =>
            {
            ...
            _zpApi.ZO_OnSniperSelected += (IPlayer player) =>
            {
            ...
            _zpApi.ZO_OnHumanWin += (bool result) =>
            {
            ...
            _zpApi.ZO_OnGameModeSelect += (string modename) =>
            {
    ...
    public void checkzombie(ICommandContext context)
    {
        ...
        if (_zpApi.ZO_IsZombie(id))
        {
            var name = _zpApi.ZO_GetZombieClassname(player);
            var valve = _zpApi.ZO_GetZombieProperties(name);
            if (valve == null)
                return;
    ...
    public void zpinf(ICommandContext context)
    {
        ...
        _zpApi.ZO_InfectPlayer(player, true);

        Core.Scheduler.DelayBySeconds(1.0f, () =>
        {

            Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test]当前丧尸名字/zombie name {_zpApi.ZO_GetZombieClassname(player)}");
            Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test]当前最大血量从配置/get max health from cfg{_zpApi.ZO_GetZombieMaxHealth(player, true)}");
            Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test]当前最大血量从当前/get max health from pawn.maxhealth {_zpApi.ZO_GetZombieMaxHealth(player, false)}");
        });
    }
    ...
    public void zpapi(ICommandContext context)
    {
        ...
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否是丧尸/Am I a zombie? {_zpApi.ZO_IsZombie(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否是母体丧尸/Am I the mother zombie? {_zpApi.ZO_IsMotherZombie(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否是复仇之神/Am I the god of vengeance? {_zpApi.ZO_IsNemesis(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否是暗杀者/Am I an assassin? {_zpApi.ZO_IsAssassin(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否是幸存者/Am I a survivor? {_zpApi.ZO_IsSurvivor(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否是狙击手/Am I a sniper? {_zpApi.ZO_IsSniper(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否是英雄/Am I a hero? {_zpApi.ZO_IsHero(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否有防化服/Do I have a scba suit? {_zpApi.ZO_PlayerHaveScbaSuit(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否处于无敌状态/Am I god mode? {_zpApi.ZO_PlayerHaveGodState(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 自己是否是无限子弹状态/Am I in a state of unlimited ammo? {_zpApi.ZO_PlayerHaveInfiniteAmmoState(id)}");
        Core.PlayerManager.SendMessage(MessageType.Chat, $"[ZPAPI测试/ZPAPI Test] 当前模式名字为/The current mode name is {_zpApi.ZO_GetCurrentModeName()}");
    }
