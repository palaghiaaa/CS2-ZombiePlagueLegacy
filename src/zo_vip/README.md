...
Standalone SwiftlyS2 CS2 plugin that brings classic **Zombie Plague VIP perks** to [ZombieOutstandingCS2](https://github.com/DeadPoolCS2/ZombieOutstandingCS2).

Integrates directly with `IZombieOutstandingAPI` for accurate zombie-state detection and infect events, and with `IEconomyAPIv1` for persistent ammo-pack rewards — no reflection hacks.
...
- **[ZombieOutstandingCS2](https://github.com/DeadPoolCS2/ZombieOutstandingCS2)** — required for zombie-state detection and infect events.
...
1. Build the project or grab `ZPOVIP.dll` from a release.
2. Copy `ZPOVIP.dll` to your SwiftlyS2 plugins folder.
3. Copy `configs/plugins/ZPOVIP/ZPOVIP.jsonc` to the same path on your server.
...
6. Restart or hot-load: `sw_plugins load ZPOVIP`.

---

## Configuration — `configs/plugins/ZPOVIP/ZPOVIP.jsonc`
...
{
  "ZPOVIP": {
    "VIPPermission":  "@zpovip/vip",   // permission flag(s), comma-separated
    "VipMenuCommand": "vip",
    "VipsListCommand": "vips",
    "ChatPrefix":     "[VIP]",
    "WalletKind":     "ammo",          // must match ZO's EconomyWalletKind
...
{
  "Permissions": [
    { "SteamID": "STEAM_1:0:12345678", "Flags": ["@zpovip/vip"] }
  ]
}
```
...
```
src/zo_vip/
├── ZPOVIP.csproj       # Project file
├── ZPOVIPPlugin.cs     # Plugin entry-point & all logic
├── ZPOVIPConfig.cs     # Strongly-typed config class
├── translations/
│   └── en.jsonc        # English strings
└── README.md           # This file

configs/plugins/ZPOVIP/
└── ZPOVIP.jsonc        # Deployable server config
```
...
Licensed under the same terms as [ZombieOutstandingCS2](../../../LICENSE).
