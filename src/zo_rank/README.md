...
A lightweight **rank and top-players** plugin for [ZombieOutstanding CS2](https://github.com/DeadPoolCS2/ZombieOutstandingCS2) built on the [SwiftlyS2](https://github.com/swiftly-solution/swiftly) framework.
...
1. Build the project or grab the compiled `ZORank.dll` from a release.  
2. Copy `ZORank.dll` to your SwiftlyS2 plugins folder.  
3. Copy `configs/plugins/ZORank/ZORankCFG.jsonc` to the same path on your server.  
...
5. Restart the server or load the plugin with `sw_plugins load ZORank`.

---

## Configuration — `configs/plugins/ZORank/ZORankCFG.jsonc`
...
```
src/zo_rank/
├── ZORank.csproj          # Project file
├── ZORank.cs              # Plugin entry-point & logic
├── ZORankCFG.cs           # Strongly-typed config class
├── translations/
│   └── en.jsonc           # English strings (add other locales here)
└── README.md              # This file

configs/plugins/ZORank/
└── ZORankCFG.jsonc        # Server-side config (deploy alongside the DLL)
```
